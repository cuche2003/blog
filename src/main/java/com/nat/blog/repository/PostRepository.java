package com.nat.blog.repository;
...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(
        value = "select post from Post post left join fetch post.user left join fetch post.blog",
        countQuery = "select count(post) from Post post"
    )
    Page<Post> findAllWithToOneRelationships(Pageable pageable);

    @Query("select post from Post post left join fetch post.user left join fetch post.blog")
